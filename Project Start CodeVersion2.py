...
##--------------------------------- Wiping Out the screen for next animation -----------------------------------------##
...
##------------------------ ---------------Flashing Wrench --------------------------------------------------------------##
    t = 1                                      #set counter
    while t<=15:                               #ran this loop for wrench 15 times
        rand_color = (random.randint(0,255),random.randint(0,255),random.randint(0,255))
        for led in range(1):
            leds = [(0,0,0)]*360               #set every LED black,
            leds[led-12] =rand_color           # leds below which make up the wrench. 
            leds[led-13] =rand_color           # assigned color at random to all Leds 
            ...
            time.sleep(0.1)
            t=t+1                               # counter keeps going up +1 everytime loop rans till it reaches 15
            
##--------------------------------- Wiping Out the screen before next animation ----------------------------------##
led = 28                                        # from LEDs 28 onwards
while led<60:                                   # move LEDs from 28 towards 60
    for rows in range (6):                      # and for every 6 rows
        leds [led + rows*60] = (0,0,0)          # set every LEDs black 
...
##------------------------ ---------------Flashing Wrench --------------------------------------------------------------##
    ...
    while t<=15:                               #ran this loop for wrench 15 times
        ...
        for led in range(1):
            leds = [(0,0,0)]*360               #set every LED black,
            leds[led-12] =rand_color           # leds below which make up the wrench. 
            leds[led-13] =rand_color           # assigned color at random
            leds[led-14] =rand_color
            leds[led-15] =rand_color
            leds[led-16] =rand_color
            leds[led-43] =rand_color
...
##----------------------------------------------------- clear wheel ---------------------------------------------------##
    led = 50                                  # from LEDs 50 0nwards
    while led<60:                             # move LEDs from 50 onwards towards 60
        for rows in range (6):                #and for every 6 rows
            leds [led + rows*60] = (0,0,0)    # set every LEDs black
        client.put_pixels(leds)
        time.sleep(.1)
...
##-----------------------------------HALF RED HALF GREEN HEARTS ---------------##


##------------------------------GREEN AND WHITE MEDICAL SIGN ------------------##
...
while True:
    value = input ( ''' RHORN'S AUTOMOTIVE SHOWROOM
                \t Type the corresponding number of your choice and press Enter.
                \t 1. Mercedes
                \t 2. Lexus
                \t 3. Chevrolet
                \t 4. SERVICE
    ...
    if value.isdigit() == True: # .isdigit()
        choice = int(value)
        if choice == 1 :                    #If number 1 is entered it calls function 1 which is Mercedes
            func1()
        elif choice ==2 :                   #If number 2 is entered it calls function 2 which is Lexus
            func2()
        elif choice ==3 :                   #If number 3 is entered it calls function 3 which is Chevrolet
            func3()
        elif choice ==4 :                   #If number 4 is entered it calls function 4 which is Service
            func4()
        elif choice ==5 :                   #If number 5 is entered it exits the simulation
            print (" You have now EXITED the online showroom.    Thank You")
            break
        else:
            print("Sorry this option is incorrect, Please enter THE NUMBER NEXT TO YOUR DESIRED CHOICE:")
        if value := chr:
