...
##--------------------------------->> Displays Mercedes Name <<---------------------------------------------------------##
    ...
    for i in MercedesName:                     # calls up every led in the list MercedesName to be lit   
        leds[i+4] = (0,230,191)                  #for leds in list MercedesName set led colour as Robin Egg Blue
        client.put_pixels(leds)
        sleep(0.15)
...
##------------------------------->> Mercedes Badge image <<------------------------------------------------------------##        
    for i in MercedesBadge:                    # calls up every led in the list MercedesBadge to be lit
        leds[i] = (0,0,255)                    #for leds in list MercedesBadge set led colour  
        client.put_pixels(leds)
        sleep(0.15)
...
##----------------------------------Germany Name-----------------------------------------------------------------------##        
    for i in GermanyName:                      # calls up every led in the list GermanName to be lit   
        leds[i+4] = (0,230,191)                  #for leds in list GermanName set led colour  
        client.put_pixels(leds)
        sleep(0.15)       
##--------------------------------- Wiping Out the screen for next animation ------------------------------------------##
    led = 0                                    # from LEDs 0 onwards
...
##--------------------------------->> German Flag <<-------------------------------------------------------------------##       
    for i in GermanFlagBackground:             # calls up every led in the list GermanFlagBackground to be lit
        leds[i] = (67,70,75)                   #for leds in list GermanFlagBackground set led colour as grey
        client.put_pixels(leds)
        sleep(.01)
...
##--------------------------------- Moving Car Animation ----------------------------------------------------------------##
    ...
    while led<27:                 
        for led in range(28):   
                ...
                leds[18+led+1] = (153,0,26)    #CAR BODY
...
##--------------------------------- Wiping Out the Car before next animation ----------------------------------------##
    led = 59                                   # from LEDs 59 onwards
    while led>20:                              # move LEDs from 59 towards 20
        for rows in range (6):                 # and for every 6 rows
            leds [led - rows*60] = (0,0,0)     # set every LEDs black 
        client.put_pixels(leds)
        time.sleep(.1)
...
##---------------------------------- Calls up Lexus Name -------------------------------------------------------------##          
    ...
    for i in LexusName:                       # calls up every led in the list LexusName to be lit   
        leds[i+10] = (255,0,0)                   #for leds in list LexusName set led colour as red
        client.put_pixels(leds)
        sleep(.01)
...
##-----------------------------------MOVING CAR ANIMATION --------------------------------------------------------------##
    ...
    while led<27:                 
        for led in range(28):   
                leds = [(0,0,0)]*360           #--Below are the LEDs and what part of the car they correspond to ------
                leds[3+led+1] = (153,0,26)     #--------
                leds[4+led+1] = (153,0,26)     #
                leds[5+led+1] = (153,0,26)     #
                ...
                leds[62+led+1] = (153,0,26)    #
                leds[63+led+1] = (153,0,26)    #
                leds[64+led+1] = (153,0,26)    #
                leds[66+led+1] = (153,0,26)    #---------
                leds[65+led+1] = (255,255,77)  #
                leds[67+led+1] = (255,255,77)  #
                leds[68+led+1] = (255,255,77)  #CAR WINDOW
                leds[69+led+1] = (255,255,77)  #
                leds[70+led+1] = (255,255,77)  #
                ...
                leds[122+led+1] = (153,0,26)   #CAR BODY
                leds[123+led+1] = (153,0,26)   #
                leds[124+led+1] = (255,255,77) #---------
                leds[125+led+1] = (255,255,77) # WINDOW
                leds[126+led+1] = (153,0,26)   #---------
                leds[127+led+1] = (255,255,77) #
                leds[128+led+1] = (255,255,77) #
                leds[129+led+1] = (255,255,77) #WINDOW
...
##------------------------------------ Calls up USA Name ----------------------------------------------------------------##
    for i in USAname:                          # calls up every led in the list USAname to be lit
        leds[i+10] = (255,0,0)                    #for leds in list USAname set led colour as red
        client.put_pixels(leds)
        sleep(.1)
##----------------------------------- Wiping Out the screen for next animation ------------------------------------------##
    led = 0                                    # from LEDs 0 onwards
...
def func4 ():
    rand_colour = (random.randint(0,255), random.randint(0,255),random.randint(0,255))
    CarHealthHeart = [309,308,307,246,250,191,185,124,128,132,72,69,67,64,5,6,10,11]
    HeartFill = [249,248,247,186,187,188,189,190,131,130,129,127,126,125,65,66,70,71]
    Service = [1,2,3,4,61,122,183,244,301,302,303,304,41,40,39,38,98,159,158,219,218,278,341,340,339,338,7,8,9,10,67,127,128,187,188,247,307,308,309,310,35,34,33,32,92,152,212,272,335,334,333,332,13,14,15,16,73,76,133,135,193,194,253,255,313,316,
               29,28,27,26,25,87,147,207,267,329,328,327,326,325,320,321,260,261,199,202,139,142,79,82,19,22]
    Pass = [12,72,132,192,252,312,13,133,14,134,15,75,135,138,198,258,318,79,199,80,200,141,201,261,321,84,144,204,324,85,205,325,86,206,326,87,207,267,327,90,150,210,330,91,211,331,92,212,332,93,213,273,333]
##---------------------------------- Wiping Out the screen for next animation -----------------------------------------##
    led = 0                                   # from LEDs 0 onwards
    while led<60:                             # move LEDs from 0 towards 60
        for rows in range (6):                # and for every 6 rows
        ...
        led = led+1
##-----------------------------------SERVICE---------------------------------------------------------------------------##
    for i in Service:                             # calls up every led in the list CarHealthHeart to be lit
        leds[i+9] = (255,0,0)                    #for leds in list CarHealthHeart set led colour as red
        client.put_pixels(leds)
        sleep(.3)
...
##------------------------ ---------------Flashing Wrench --------------------------------------------------------------##
    ...
    while t<=15:                               #ran this loop for wrench 15 times
        rand_color = (random.randint(0,255),random.randint(0,255),random.randint(0,255)) ## The range of the random RGB color values 
        for led in range(1):
...
##--------------------FLASHING BATTERY ANIMATION -------------------------------------------------------------##
    t = 1                                       #sets counter
    led = 0
    while t<50:                                 #sets counter for 50, rans loop x50 times
        rand_color = (random.randint(255,255),random.randint(99,215),random.randint(0,80)) ##range of the random RGB color values
        for led in range(1):   
            leds = [(0,0,0)]*360    
...
##--------------------------------------ENGINE WARNING LIGHT ------------------------------------------------##
        rand_color = (random.randint(255,255),random.randint(99,215),random.randint(0,80)) ##range of the random RGB color values
        for led in range(1):   
            leds[48-led] = rand_color             ##below are the leds to which the color range would be applied
...
##--------------------------------------------FAULT -----------------------------------------------------------##      
        ...
        rand_color = (random.randint(220,255),random.randint(0,69),random.randint(0,60)) ##range of the random RGB color values
        for led in range(1):
            leds[19-led] = rand_color        ##below are the leds to which the color range would be applied
            ...
            client.put_pixels(leds)
            time.sleep(.1)
            t=t+1                              ## counter keeps going up +1 everytime loop rans till it reaches 50
        client.put_pixels(leds)   
##--------------------------------- Wiping Out the screen for next animation -------------------------------------------##
    led = 0                                    # from LEDs 0 onwards
    while led<60:                              # move LEDs from 0 towards 60
        ...
        led = led+1
##---------------------------------------Car Health Heart --------------------------------------------------------------##
##Heart health starts off red(showing faluty), then moves to light orange (improving) then green (good haelth /health restored)
    for i in CarHealthHeart:                   # calls up every led in the list CarHealthHeart to be lit
        leds[i] = (255,0,0)                    #for leds in list CarHealthHeart set led colour as red
        client.put_pixels(leds)
        sleep(.1)

    for i in HeartFill:                        #calls up every led in the list HeartFill to be lit
        leds[i] = (255,0,0)                    #for leds in list HeartFill set led colour as red
        client.put_pixels(leds)
        sleep(.1)

    for i in CarHealthHeart:                   #calls up every led in the list CarHealthHeart to be lit
        leds[i+20] = (255,85,0)                #for leds in list CarHealthHeart set led colour as red
        client.put_pixels(leds)
        sleep(.1)

    for i in HeartFill:                        #calls up every led in the list HeartFill to be lit
        leds[i+20] = (255,85,80)               #for leds in list HeartFill set led colour as light red
        client.put_pixels(leds)
        sleep(.1)

    for i in CarHealthHeart:                   #calls up every led in the list CarHealthHeart to be lit
        leds[i+40] = (255,90,0)                #for leds in list CarHealthHeart set led colour as red
        client.put_pixels(leds)
        sleep(.1)

    for i in HeartFill:                        #calls up every led in the list HeartFill to be lit
        leds[i+40] = (255,170,0)               #for leds in list HeartFill set led colour as transition from red to amber to green
        client.put_pixels(leds)
        sleep(.1)

    for i in CarHealthHeart:                   #calls up every led in the list CarHealthHeart to be lit
        leds[i] = (170,255,0)                  #for leds in list CarHealthHeart set led colour as transition from red to amber to green
        client.put_pixels(leds)
        sleep(.1)
    
    for i in HeartFill:                        #calls up every led in the list HeartFill to be lit
        leds[i] = (140,255,25)                 #for leds in list HeartFill set led colour as transition to green
        client.put_pixels(leds)
        sleep(.1)

    for i in CarHealthHeart:                   #calls up every led in the list CarHealthHeart to be lit
        leds[i+20] = (65,255,25)               #for leds in list CarHealthHeart set led colour as green
        client.put_pixels(leds)
        sleep(.1)

    for i in HeartFill:                        #calls up every led in the list HeartFill to be lit
        leds[i+20] = (65,255,25)               #for leds in list HeartFill set led colour as green
        client.put_pixels(leds)
        sleep(.1)

    for i in CarHealthHeart:                   # calls up every led in the list CarHealthHeart to be lit
        leds[i+40] = (0,255,0)                 #for leds in list CarHealthHeart set led colour as green
        client.put_pixels(leds)
        sleep(.1)

    for i in HeartFill:                        # calls up every led in the list HeartFill to be lit
        leds[i+40] = (0,255,0)                 #for leds in list HeartFill set led colour as green
        client.put_pixels(leds)
        sleep(.1)
##--------------------------------- Wiping Out the screen for next animation -------------------------------------------##
    led = 0                                    # from LEDs 0 onwards
    while led<60:                              # move LEDs from 28 towards 60
        ...
        led = led+1
##----------------------------------------PASS -------------------------------------------------------------------------##
    for i in Pass:                             #calls up every led in the list Pass to be lit
        leds[i] = (0,255,0)                    #for leds in list Pass set led colour as green
        client.put_pixels(leds)
        sleep(.1)
    for i in CarHealthHeart:                   #calls up every led in the list CarHealthHeart to be lit
        leds[i+40] = (0,255,0)                 #for leds in list CarHealthHeart set led colour as green
...
while True:
    ...
    if value.isdigit() == True: # .isdigit()
        ...
        if choice == 1 :                     #If number 1 is entered it calls function 1 which is Mercedes
            ...
            print (" You have now EXITED RHORN'S AUTOMOTIVE SHOWROOM.    Thank You")
            break
        else:
